    ...
    <el-main
      ...
      <div style="width: 100%;">
        <el-table
          ...
          <el-table-column prop="completionTime" label="完成时间" min-width="15%" sortable>
          </el-table-column>
          <el-table-column min-width="15%">
            <template slot="header" slot-scope="scope">
              <el-input v-model="search" size="mini" placeholder="维修内容">
    ...
    <el-dialog
      ...
      <el-card :body-style="{ padding: '10px' }" >
        ...
        <div style="float:left;margin: 10px auto auto 20px;padding: 10px;text-align: left">
          ...
          <div style="margin-top: 20px"><span>完成时间</span><span style="margin-left: 20px">{{ this.repair.completionTime}}</span></div>
        </div>
      </el-card>
  ...
  methods: {
    ...
    setReceived(repair) {
      this.$confirm('确定接单吗?', '提示', {
        ...
        this.repair = repair
        this.repair.state = "维修完成"
        this.repair.repairman = this.$store.getters.getUserId
